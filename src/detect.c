...
int detect_lines1(pix * p, int x0, int y0, int dx, int dy)
{
  ...
  int i, jj, /*j2,*/ y, yy, my, mi, /* mc,*/ i1, i2, i3, i4,
      ...
      mt1, mt2, mt4, mtc; /* 2017 from global triple-pxH */
  struct box *box2, *box3, *box4;  /* box3 is for verbose / debugging */
  ...
  //  ToDo: find 2 neighbouring chars per line pH Hp + third x or p or H to verify
  //     to set default heights
#if 1
  /* this is only for test runs */
  if (job->cfg.verbose & 16)
    fprintf(stderr," search pxH-like-triple using   my=%4d\n#",my);
#endif
  mt1= mt2= mt4= mtc= 0; /* averages m1..m4 to base-line m2 */
  /* O(n^3)-loop = slow = bad */
  for_each_data(&(job->res.boxlist)) { /* 2017-07 added , ToDo: use ma1-ma4 */
    box2 = (struct box *)list_get_current(&(job->res.boxlist));
    if (box2->c == PICTURE) continue;
    for_each_data(&(job->res.boxlist)) {
      box3 = (struct box *)list_get_current(&(job->res.boxlist));
      if (box3->c == PICTURE) continue;
      if (box2==box3) continue;
      if (box3->y1 < box2->y0) continue; /* other above line */
      if (box3->y0 > box2->y1 + 2*my) break;    /* other below line */
      if (box3->x1 < box2->x0 - 8*my) continue; /* far away left char */
      if (box3->x0 > box2->x1 + 8*my) continue; /* far away right char */
      if (box2->y0 != box3->y0
       || box2->y1 <= box3->y1) continue; /* px-like */
      for_each_data(&(job->res.boxlist)) {
        box4 = (struct box *)list_get_current(&(job->res.boxlist));
        if (box4->c == PICTURE) continue;
        if (box2==box4 || box3==box4) continue;
        if (box4->y1 < box2->y0) continue; /* other above line */
        if (box4->y0 > box2->y1 + 2*my) break;    /* other below line */
        if (box4->x1 < box2->x0 - 8*my) continue; /* far away left char */
        if (box4->x0 > box2->x1 + 8*my) continue; /* far away right char */
        if (box3->y1 != box4->y1
         || box3->y0 <= box4->y0) continue; /* pxH-like */
//        if (job->cfg.verbose & 16)
//          fprintf(stderr,"  found pH-like-pair xy %3d %3d  %3d %3d  %3d %3d\n#",
//                  box2->x0,box2->y0,box3->x0,box3->y0,box4->x0,box4->y0);
        mtc++;
        mt1+= box4->y0 - box4->y1; /* neg. H-line */
        mt2+= box3->y0 - box3->y1; /* neg. x-line */
        mt4+= box2->y1 - box3->y1; /* pos. p-line */
      } end_for_each(&(job->res.boxlist));
    } end_for_each(&(job->res.boxlist));
  } end_for_each(&(job->res.boxlist));
  if (mtc==0) {mt1=mt2=mt4=0;mtc=1;}
  if (job->cfg.verbose & 16)
    fprintf(stderr,"  found %4d pxH-like-triples m124=%4.1f %4.1f %4.1f\n#",
            mtc, (mt1*1.)/mtc, (mt2*1.)/mtc, (mt4*1.)/mtc);
  if (job->cfg.verbose & 16)
    fprintf(stderr,"  found %4d pxH-like-triples m124=%4d %4d %4d\n#",
            mtc, (mt1-mtc+1)/mtc, (mt2-mtc+1)/mtc, (mt4+mtc-1)/mtc);
  mt1=(mt1-mtc+1)/mtc; /* round up! */
  mt2=(mt2-mtc+1)/mtc;
  mt4=(mt4+mtc-1)/mtc; /* added 2017-07 ToDo: use for short lines later */

  m4pre=m3pre=y0;   /* lower bond of upper line */
  // ToDo: better function for scanning line around a letter ???
  //       or define lines around known chars "eaTmM"
  for (/*j2 =*/ y = y0; y < y0 + dy; y++) { /* line by line loop */
    ...
    //    ToDo: better use triples with 2 chars same line, one above or below
    ...
    // ToDo: histogramm of highs + shrink to 3-4 main-peaks
    //       start histogramm with tripples xHx or xpx and get p-length
    //       assuming constant font + certainty?
    //       p_len[4]={} p_line-base_line + certainty[4]={0,0,0,0}
    //     what is the simplest good way?
	...
	&& (box2->y1 - box2->y0 + 1) > 3           // 4x6 lowercase=4
         ...
         )
      {
        ...
        if (box2->y0 > m2 + yy) {
          ...
          if (job->cfg.verbose & 16)
            fprintf(stderr,"  set m2= %3d %+3d yy= %2d  reason1 xy= %4d %4d\n#",
        ...
        if (box2->y1 > m4 + yy && (my>6 || box2->y1 < m3+my)) {
          ...
          if (job->cfg.verbose & 16)
            fprintf(stderr,"  set m4= %3d %+3d yy= %2d  reason1 xy= %4d %4d\n#",
         ...
         || num_cross(box2->x0,box2->x1,(box2->y0+box2->y1)/2,(box2->y0+box2->y1)/2,box2->p,job->cfg.cs)>2 )
        {
          ...
          if (job->cfg.verbose & 16)
            fprintf(stderr,"  set m3= %3d %+3d yy= %2d  reason1 xy= %4d %4d\n#",
    ...
    // ToDo: use holes for m1==m2 and m3==m4 pqbdoaegABDOPQR0689%&#
    // or ij-dots + umlauts + .,;?! ??
    ...
    /* ToDo17 correct m3==m4 or m1==m2 using mt1..mt4 */
    ...
    // tmp17/0506_clean7x9c.png dbtlhk lower than ABCD
    if (i1)  m1 = (ma1+i1/4) / i1; /* best rounded, H-line */
    if (i2)  m2 = (ma2+i2/2) / i2; /* x-line */
    if (i3)  m3 = (ma3+i3-1) / i3; /* round up, base-line */
    if (i4)  m4 = (ma4+i4-1) / i4; /* p-line */
...
//	  && box2->x0 <= lines->x1[i] + 6 + 4*job->res.avX // in box ?
...
#if 1
    /* this is only for test runs, m1=H-line m2=x-line m3=base m4=p-line */
    if (job->cfg.verbose & 16)
    ...
    if (jj>0 && job->cfg.verbose) {
      /* m1=cap_high=H-line m2=median=x-line m3=base m4=descent=p-line */
      /* see https://en.wikipedia.org/wiki/Baseline_%28typography%29 */
      fprintf(stderr,"  L%02d w= %3d y= %3d m= %4d %+3d %+3d %+3d (H,x,base,p)\n#",i,lines->wt[i],y,m1,m2-m1,m3-m1,m4-m1);
    ...
    //  ToDo17 use Hxp-triple values mt1,mt2,mt4 to fix jj!=0 lines
    //         sample: tmp17/0506_clean7x9a.png (short lines)
    ...
    if (mt1<mt2 && mt2<0 && mt4>0 && jj==3) { /* valid Hxp-tripples found, use here */
      //fprintf(stderr,"  trying pxH-fix by Hxp %3d %3d   0 %3d\n#",
       //     mt1, mt2, mt4);
      if (m4-m1+mt2<=my/8) /* acemnorsuvwxz */
        { jj=0; m1=m3+mt1; m4=m3+mt4; } /* 2017-08 */
      if (m4-m1+mt1<=my/8) /* ABCDEF */
        { jj=0; m2=m3+mt2; m4=m3+mt4; } /* 2017-08 but mt1 == mt4-mt2? */
      if (m4-m1+mt2-mt4<=my/8) /* pqy */
        { jj=0; m1=m2-mt2+mt1; m3=m2-mt2; } /* 2017-08 */
     // if (jj==0) fprintf(stderr,"       fixed by Hxp to %4d %3d %3d %3d\n#",
    ...
    {				// empty space between lines
      lines->m4[i] = m4; // p-line (descent)
      lines->m3[i] = m3; // base-line
      lines->m2[i] = m2; // x-line (median)
      lines->m1[i] = m1; // H-line
  ...
  lines->num = i;
  if (job->cfg.verbose)
    fprintf(stderr, "# detect_lines1.num_lines= %d", lines->num-1);
  return 0;
}
