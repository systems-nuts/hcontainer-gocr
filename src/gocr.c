...
#define PIC_SIZE 35167 // make sure it is the same as client.c PIC_SIZE
#include <string.h>
#include <dirent.h>
...
static void help(void) {
  ...
  fprintf(stderr, /* string length less than 509 bytes for ISO C89 */
	  ...
	  "\n");
  fprintf(stderr, " website: http://www-e.uni-magdeburg.de/jschulen/ocr/\n");
/* fprintf(stderr, " website: http://jocr.sourceforge.net/ (2000-2013)\n"); */
  exit(0);
}
...
static void process_arguments(job_t *job, int argn, char *argv[])
{
  ...
  /* process arguments */
  for (i = 1; i < argn; i++) {
    int iskip= 0; /* 2017-03-26 allow -v33 (iskip=0) or -v 33 (iskip=1) */
    if (strcmp(argv[i], "--help") == 0
     || strcmp(argv[i], "-h")     == 0) help(); /* and quits  2018-09 -h */
    if (strcmp(argv[i], "--version") == 0
     || strcmp(argv[i], "-V")        == 0) { /* and quits  2018-09 */
       printf(version_string "-" release_string "\n"); exit(0);}
    if (argv[i][0] == '-' && argv[i][1] != 0) {
      if (i + 1 < argn && argv[i][2]=='\0') { s1 = argv[i + 1]; iskip=1; }
      else if (argv[i][2]!='\0')              s1 = argv[i]+2;
           else {
...
#define SERVER_ON
	...
	job->cfg.db_path=s1; break;
      case 'o': /* output file */
	if (s1[0] == '-' && s1[1] == '\0') { } /* default = stdout */
...
int tcpserver(){
    ...
    int kilo_bytes = (1024);
    char *buffer = (char *) malloc(PIC_SIZE);//314 * kilo_bytes * kilo_bytes);
    char *buf_ptr, *ack = "AKC", *f_ack="NAK";
    char *strarray[382];
    DIR *d;
    struct dirent *dir;
    int writesize = PIC_SIZE; //314 * kilo_bytes * kilo_bytes;
    clock_t start, end;
    FILE *fd;
    int rec_left;
    struct timeval tv;
...
#ifdef SERVER_ON
  printf("welcome to hcontainer server model \n");
  tcpserver();
#else  
